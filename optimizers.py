...
class SGD(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m in zip(parameters, gradients, self.state['m']):
            ...
            """
            grad_L2_reg = grad + self.weight_decay * param
            np.add(self.momentum * m, grad_L2_reg, out=m)
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m, v in zip(parameters, gradients, self.state['m'], self.state['v']):
            ...
            """
            grad_L2_reg = grad + self.weight_decay * param
            np.add(self.beta1 * m, (1. - self.beta1) * grad_L2_reg, out=m)
            np.add(self.beta2 * v, (1. - self.beta2) * np.square(grad_L2_reg), out=v)
            m_hat = m / (1. - self.beta1 ** t)
            v_hat = v / (1. - self.beta2 ** t)
            np.add(param, -1. * self.lr * (m_hat / (np.sqrt(v_hat) + self.eps)), out=param)
